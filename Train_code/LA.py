...
from transformers import set_seed, AutoTokenizer, AutoModelForCausalLM, get_cosine_schedule_with_warmup
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def _build_ids(self, messages: List[Dict[str, str]], add_generation_prompt: bool) -> List[int]:
        if hasattr(self.tokenizer, 'apply_chat_template') and getattr(self.tokenizer, 'chat_template', None):
            try:
                ids = self.tokenizer.apply_chat_template(messages, add_generation_prompt=add_generation_prompt, tokenize=True, return_tensors=None)
            except TypeError:
                ids = self.tokenizer.apply_chat_template(messages, add_generation_prompt=add_generation_prompt, tokenize=True)
            return ensure_list_of_int(ids)
        text = self.template.render(messages=messages, bos_token=self.tokenizer.bos_token or '', add_generation_prompt=add_generation_prompt)
    ...
    def build_features(self, da: Dict[str, Any]) -> Dict[str, List[int]]:
        q = da['prompt']
        a = da['ground_truth']
        full_ids = self._build_ids([{'role': 'user', 'content': q}, {'role': 'assistant', 'content': a}], add_generation_prompt=False)
        prompt_ids = self._build_ids([{'role': 'user', 'content': q}], add_generation_prompt=True)
    ...
    def collate_fn(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        return {'input_ids': torch.LongTensor(padded_input_ids), 'labels': torch.LongTensor(padded_labels), 'attention_mask': torch.LongTensor(attention_masks)}
...
def save_hf_checkpoint_huatuo_copy(accelerator: Accelerator, model, tokenizer: AutoTokenizer, save_dir: str, base_model_dir: str) -> None:
    ...
    if accelerator.is_main_process:
        ...
        unwrapped.save_pretrained(save_dir, save_function=accelerator.save, state_dict=state_dict, safe_serialization=True)
...
def save_epoch_snapshot_to_best_dir(accelerator: Accelerator, saved_ckpt_dir: str, snapshot_dir: str) -> None:
    accelerator.wait_for_everyone()
    if accelerator.is_main_process:
...
def train(args: argparse.Namespace) -> None:
    ...
    if accelerator.is_main_process:
        swanlab.init(project=args.swanlab_project, experiment_name=args.swanlab_experiment_name, config=vars(args))
    ...
    optimizer_grouped_parameters = [{'params': [p for n, p in model.named_parameters() if not any((k in n for k in no_decay_keywords))], 'weight_decay': args.weight_decay}, {'params': [p for n, p in model.named_parameters() if any((k in n for k in no_decay_keywords))], 'weight_decay': 0.0}]
    optimizer = torch.optim.AdamW(optimizer_grouped_parameters, lr=args.learning_rate)
    train_dataset = TrainDataset(args, tokenizer)
    train_dataloader = DataLoader(train_dataset, batch_size=args.train_bsz_per_gpu, shuffle=True, drop_last=True, collate_fn=train_dataset.collate_fn, num_workers=args.num_workers, pin_memory=True)
    ...
    for epoch in range(args.n_epochs):
        ...
        updates_in_epoch = 0
        it = tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc=f'Epoch {epoch}') if accelerator.is_main_process else enumerate(train_dataloader)
        for _, batch in it:
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    it.set_postfix(loss=round(loss_step, 4), ema=round(ema_loss, 4), acc=round(acc_step, 4), lr=lr_scheduler.get_last_lr()[0])
                    if step_log_interval is not None and (updates_in_epoch % step_log_interval == 0 or updates_in_epoch == num_update_steps_per_epoch):
                        swanlab.log({'step_loss': float(loss_step), 'step_acc': float(acc_step), 'ema_loss': float(ema_loss), 'lr': float(lr_scheduler.get_last_lr()[0]), 'epoch': epoch, 'updates_in_epoch': updates_in_epoch}, step=global_step)
                if ckpt_save_interval is not None and (updates_in_epoch % ckpt_save_interval == 0 or updates_in_epoch == num_update_steps_per_epoch):
                    accelerator.wait_for_everyone()
                    ckpt_name = f'checkpoint-epoch-{epoch}-step-{updates_in_epoch}'
                    ckpt_dir = os.path.join(args.output_dir, ckpt_name)
                    save_hf_checkpoint_huatuo_copy(accelerator=accelerator, model=model, tokenizer=tokenizer, save_dir=ckpt_dir, base_model_dir=args.model_path)
        ...
        save_hf_checkpoint_huatuo_copy(accelerator=accelerator, model=model, tokenizer=tokenizer, save_dir=epoch_dir, base_model_dir=args.model_path)
        ...
        if accelerator.is_main_process:
            ...
            swanlab.log({'epoch': epoch, 'epoch_loss': float(epoch_loss), 'epoch_acc': float(epoch_acc), 'epoch_global_step': global_step}, step=global_step)
        accelerator.wait_for_everyone()
    if accelerator.is_main_process:
        accelerator.print('\n' + '=' * 60)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--learning_rate', default=5e-06, type=float)
