...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def _render_text(self, messages: List[Dict[str, str]], add_generation_prompt: bool) -> str:
        return self.template.render(messages=messages, bos_token=self.tokenizer.bos_token or '', add_generation_prompt=add_generation_prompt)

    def build_features(self, da: Dict[str, Any]) -> Dict[str, List[int]]:
        q = da['prompt']
        a = self.get_response(da)
        full_text = self._render_text([{'role': 'user', 'content': q}, {'role': 'assistant', 'content': a}], add_generation_prompt=False)
        full_ids = self.tokenizer.encode(full_text, add_special_tokens=False)
        prompt_text = self._render_text([{'role': 'user', 'content': q}], add_generation_prompt=True)
...
def train(args: argparse.Namespace) -> None:
    ...
    accelerator.print(f'steps_per_epoch={steps_per_epoch}, num_update_steps_per_epoch={num_update_steps_per_epoch}, num_training_steps={num_training_steps}, warmup_steps={warmup_steps}')
    lr_scheduler = get_cosine_schedule_with_warmup(optimizer, num_warmup_steps=warmup_steps, num_training_steps=num_training_steps)
    ...
    for epoch in range(args.n_epochs):
        ...
        for _, batch in it:
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    it.set_postfix(loss=round(loss_step, 4), ema=round(ema_loss, 4), acc=round(acc_step, 4), lr=lr_scheduler.get_last_lr()[0], up=updates_in_epoch)
        ...
        if accelerator.is_main_process:
            ...
            swanlab.log({'epoch': epoch_num, 'epoch_loss': float(epoch_loss), 'epoch_acc': float(epoch_acc), 'epoch_global_step': global_step}, step=global_step)
            if epoch_num % 2 == 0:
