...
class DeepseekOCRProcessor(ProcessorMixin):
    ...
    def process_one(self, prompt: str, images: List, inference_mode: bool=True, **kwargs):
        ...
        return {'input_ids': input_ids, 'pixel_values': pixel_values, 'images_crop': images_crop, 'images_seq_mask': images_seq_mask, 'images_spatial_crop': images_spatial_crop, 'num_image_tokens': num_image_tokens}

    def __call__(self, *, prompt: str, images: List, inference_mode: bool=True, **kwargs):
        prepare = self.process_one(prompt=prompt, images=images, inference_mode=inference_mode)
        return prepare

    def tokenize_with_images(self, images: List[Image.Image], bos: bool=True, eos: bool=True, cropping: bool=True):
        conversation = PROMPT
        ...
        for text_sep, image in zip(text_splits, images):
            ...
            if num_width_tiles > 1 or num_height_tiles > 1:
                tokenized_image += ([self.image_token_id] * (num_queries * num_width_tiles) + [self.image_token_id]) * (num_queries * num_height_tiles)
