...
def merge_similar_entries_in_document(entries, model, threshold=0.65):
    ...
    list_item_pattern = re.compile(
        r"^\s*(" r"\d+\s*\." r"|\(\s*[a-zA-Z\u0900-\u097F]+\s*\)" r"|[\w\s]+—" r")",
        re.UNICODE,
    )
    ...
    while i > 0:
        ...
        if similarity >= threshold:
            merged_text = (
                (result_data[prev_idx_in_result].get("text") or "").strip()
                + " "
                + (result_data[current_idx_in_result].get("text") or "").strip()
            ).strip()
            ...
            merge_logs.append(
                {
                    ...
                    "from_index": current_item["index"],
                    "to_index": prev_item["index"],
                    "final_page_idx": merged_page_idx,
                }
            )
