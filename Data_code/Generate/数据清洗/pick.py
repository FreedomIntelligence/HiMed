...
def is_ttff(entry: dict) -> bool:
    ...
    return (
        is_hindi is True and
        is_medical is True and
        has_ambiguity is False and
        is_title is False
    )
...
def process_file(input_path: Path, picked_path: Path, abandoned_path: Path):
    ...
    print(
        f"{input_path.name}: picked={len(picked)}, "
        f"abandoned={len(abandoned)}, total={len(data)}"
    )
    return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Pick entries from JSON files based on boolean flags"
    )
    parser.add_argument(
        "--input-folder",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Folder for picked entries (TTFF)"
    )
    parser.add_argument(
        "--abandoned-folder",
        type=str,
        required=True,
        help="Folder for abandoned entries (not TTFF)"
    )
    ...
    process_all(
        ...
        picked_folder=args.picked_folder,
        abandoned_folder=args.abandoned_folder
    )
