...
def split_pages(text: str) -> List[str]:
    page_split_mark = r"\s*<--- Page Split --->\s*"
...
def page_text_to_blocks(page_text: str, page_idx: int) -> List[dict]:
    ...
    max_blank_lines = 3
    blanks_rx = re.compile(r"\n{" + str(max_blank_lines) + r",}", re.UNICODE)
...
def get_root_mmd_files(input_dir: Path) -> List[Path]:
    ...
    for p in input_dir.glob("*"):
        if (
            p.is_file()
            and p.suffix.lower() == ".mmd"
            and not p.name.lower().endswith("_det.mmd")
        ):
            mmd_files.append(p)
...
def process_one_mmd(src_file: Path, output_dir: Path, force_overwrite: bool) -> bool:
    ...
    try:
        ...
        print(
            f"[INFO] Parsed {src_file.name}: {len(pages_json)} pages "
            f"(aligned with original PDF page count)"
        )

        dst_file.write_text(
            json.dumps(pages_json, ensure_ascii=False, indent=4),
            encoding="utf-8",
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert MMD files to JSON structure"
    )
    parser.add_argument(
        "--input-dir",
        type=str,
