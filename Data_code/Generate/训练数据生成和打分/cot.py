...
def load_templates_pool(xlsx_path: Path) -> Dict[Tuple[str, str], List[str]]:
    ...
    if not (col_subj and col_qtpl and col_qtype):
        raise ValueError(
            f"模板表缺关键列，需要 Subject + question_templete + qtype。现有列：{list(df.columns)}"
        )
...
Q_RX = re.compile(r"<q>(.*?)</q>", re.S | re.I)
A_RX = re.compile(r"<a>(.*?)</a>", re.S | re.I)
COT_RX = re.compile(r"<cot>(.*?)</cot>", re.S | re.I)
DIFF_BLOCK_RX = re.compile(r"<(EASY|MEDIUM|HARD)>(.*?)</\\1>", re.S | re.I)
...
TYPE_PROMPTS = {
    "MCQ": f"""You are an expert question writer for traditional medicine.
...
def build_gen_prompt(text: str,
    ...
    lang_note = (
        "The paragraph is in Hindi (Devanagari script); use Hindi for Q/A/CoT."
        if lang == "hi"
        else "The paragraph is not in Devanagari; use English for Q/A/CoT."
    )
...
def parse_multi_qa_cot(text: str) -> List[Dict]:
    ...
    for diff in ["EASY", "MEDIUM", "HARD"]:
        m = re.search(rf"<{diff}>(.*?)</{diff}>", t, re.S | re.I)
        if not m:
            continue
        block = m.group(1)
        q = Q_RX.search(block)
...
def generate_qa_for_entry(entry: Dict) -> List[Dict]:
    ...
    if not ok:
        ...
        ok, content = openai_tool.get_respons(
            build_gen_prompt(text, subject, qtype, question_template, few_shot, relaxed=True),
            model=MODEL_NAME,
        )
        if not ok:
            return []
    ...
    if not qa_list:
        ok_relax, content_relax = openai_tool.get_respons(
            build_gen_prompt(text, subject, qtype, question_template, few_shot, relaxed=True),
            model=MODEL_NAME,
        )
        if ok_relax:
