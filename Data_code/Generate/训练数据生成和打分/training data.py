...
def load_templates_pool(xlsx_path: Path) -> Dict[Tuple[str, str], List[str]]:
    ...
    if "Hindi" not in df.columns and "English" not in df.columns:
        raise ValueError(
            f"模板表缺少 Hindi/English 文本列，现有列: {list(df.columns)}"
        )
...
def process_single_text_id(
    blk: Dict,
    idx: int,
    rid_map: Dict[str, str],
    processed_text_ids: set,
    text_id_entry_count: Dict[str, int],
    templ_pool: Dict[Tuple[str, str], List[str]],
    ex_pool: Dict[Tuple[str, str], List[Dict]]
    ...
    for subject in subject_list:
        for qtype in type_list:
            ...
            base_entry = {
                ...
                "subject": subject,
                "type": qtype,
                "question_templete": qtpl,
                "few_shot": fs,
                "metadata": {
                    "raw_id": raw_id3,
                    "text_id": text_id,
                    "page_range": page_range,
                    "raw_id_str": raw_id_str,
                },
            }
            ...
            for qa_idx, qa in enumerate(qa_list):
                ...
                entry = {
                    "id": _id,
                    **{k: v for k, v in base_entry.items() if k != "few_shot"},
                    "metadata": {
                        **base_entry["metadata"],
                        "entry_id": entry_id,
                        "difficulty": difficulty,
                    },
                    "question": sanitize_output_text(qa.get("question", "")),
                    "answer": sanitize_output_text(qa.get("answer", "")),
                    "cot": sanitize_output_text(qa.get("cot", "")),
                }
    ...
    print(
        f"  - text_id={text_id}, subjects={subject_list}, "
        f"types={type_list}, entries={entry_counter}, total_entries={len(text_entries)}"
    )
    return text_id, text_entries, False

def process_results_good(
    src: Path, 
    dst: Path,
    instr_xlsx_path: Path,
    examples_xlsx_path: Path,
    sleep_between_req: float = 0.5,
            ...
            if known_subjects:
                ...
                if actual_count >= expected_entry_count:
                    ...
                    all_entries = [e for e in all_entries 
                                  if ((e.get("metadata") or {}).get("text_id")) != text_id]
                    incomplete_count += 1
    ...
    if parallel_workers > 1 and len(blocks_to_process) > 1:
        ...
        def process_and_save(block_data):
            idx, blk = block_data
            text_id, text_entries, should_skip = process_single_text_id(
                blk, idx, rid_map, processed_text_ids, text_id_entry_count, templ_pool, ex_pool
            )
        ...
        with ThreadPoolExecutor(max_workers=parallel_workers) as executor:
            future_to_block = {executor.submit(process_and_save, block_data): block_data 
                ...
                try:
                    ...
                    if not should_skip and text_entries:
                        ...
                        if len(batch_results) >= REPORT_BATCH_SIZE:
                            ...
                            print(f"[{file_name}] 批次 {batch_number}: 完成 {len(batch_results)} 个 text_id, "
                                  f"总计 {batch_entries_sum} entries, "
                                  f"总进度: {completed_count}/{len(blocks_to_process)}")
                            batch_results = []
                            batch_number += 1
                except Exception as e:
            ...
            if batch_results:
                ...
                print(f"[{file_name}] 批次 {batch_number}: 完成 {len(batch_results)} 个 text_id, "
                      f"总计 {batch_entries_sum} entries, "
                      f"总进度: {completed_count}/{len(blocks_to_process)} (全部完成)")
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert one results_good *.good.json into step6 JSON."
    )
    parser.add_argument(
        "--input",
        type=str,
        required=True,
        help="Path to *.good.json",
    )
    parser.add_argument(
        "--output",
        type=str,
        required=True,
        help="Path to output step6 JSON",
    )
    parser.add_argument(
        "--instr-xlsx",
        type=str,
        required=True,
        help="Path to instruction template Excel file",
    )
    parser.add_argument(
        "--examples-xlsx",
        type=str,
        required=True,
        help="Path to few-shot examples Excel file",
    )
    parser.add_argument(
        "--sleep-between-req",
        type=float,
    ...
    process_results_good(
        src=input_json,
        dst=output_json,
        instr_xlsx_path=instr_xlsx_path,
        examples_xlsx_path=examples_xlsx_path,
