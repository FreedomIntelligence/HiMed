# translation_api_optimized.py
# Optimized version with batch processing and GPU utilization
# Public API: translate_paragraph(text: str) -> str
#            translate_paragraphs(texts: List[str]) -> List[str]
...
class _Config:
    """Centralized configuration (internal)"""
    # Paths
    ...
    BATCH_SIZE = 8 # Sentences per batch (adjust based on GPU memory)
    
    # Dynamic batching
    USE_DYNAMIC_BATCHING = True  # Group sentences by similar length
    LENGTH_BUCKET_SIZE = 16  # Token range for grouping
...
# ============================================================================
# Sentence splitter (BlingFire-backed, keeps old interface)
# ============================================================================

class _NLTKManager:
    """Manages sentence tokenization (now using BlingFire instead of NLTK)"""
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        BlingFire's text_to_sentences returns a single string where
        ...
        # Split lines + strip, filter empty
        sents = [s.strip() for s in s_block.split("\n") if s.strip()]
        return sents


# ============================================================================
# Pre-translation lexicon (internal)
# ============================================================================

class _PreTranslationLexicon:
    """Handles English → Hindi pre-translation mappings (internal)"""
    ...
    def _build_term_regex(self, eng: str) -> str:
        """Build a robust regex for one English term"""
    ...
    def _load(self):
        ...
        try:
            ...
            if term_patterns:
                ...
                right_boundary = r"(?=(?:'s|'s)?(?![A-Za-z0-9_]))"
    ...
    def apply(self, text: str) -> Tuple[str, Dict[str, Tuple[str, str]]]:
        """Apply English→Hindi substitutions, record used pairs"""
        if not self.pattern:
            return text, {}
    ...
    def annotate(hin_text: str, used: Dict[str, Tuple[str, str]]) -> str:
        """Add English hints back as 'Hindi (English)' for all occurrences"""
        if not used:
            return hin_text
        ...
        for _, (eng_original, hin) in items:
            pattern = re.compile(re.escape(hin) + r"(?!\s*\()", flags=0)
...
class _Postprocessor:
    """Postprocessing pipeline (internal)"""
...
# ============================================================================
# Optimized NLLB translator with batch processing (internal)
# ============================================================================

class _NLLBTranslator:
    """Optimized NLLB model wrapper with batch support (internal)"""
    ...
    def _load(self):
        ...
        try:
            ...
            # ---- Device map debug: check for CPU offload ----
            device_map = getattr(self.model, "hf_device_map", None)
            if device_map is not None:
                print("HF device map:", device_map)
            else:
                # Fallback info（老版本 transformers 可能没有 hf_device_map）
            ...
            # Get target language token ID once during initialization
    ...
    def _get_target_lang_id(self):
        """Get target language token ID with multiple fallback methods"""
        try:
            # Method 1: lang_code_to_id
    ...
    def translate_batch(self, texts: List[str]) -> List[str]:
        """Batch translation - main optimization"""
        if not texts:
            return []
...
# ============================================================================
# Optimized sentence translator with batch processing (internal)
# ============================================================================

class _SentenceTranslator:
    """Optimized sentence-by-sentence translation with batching (internal)"""
    ...
    def _create_batches(self, sentences: List[str]) -> List[List[int]]:
        """Create batches of sentence indices"""
        if not _Config.USE_DYNAMIC_BATCHING:
            return [
                list(range(i, min(i + _Config.BATCH_SIZE, len(sentences))))
                for i in range(0, len(sentences), _Config.BATCH_SIZE)
            ]
        ...
        for idx, length in indexed_lengths:
            if len(current_batch) >= _Config.BATCH_SIZE or \
               (current_batch and length > current_tokens + _Config.LENGTH_BUCKET_SIZE):
    ...
    def translate(self, text: str, item_idx: int = 0) -> str:
        """Translate paragraph with batch processing"""
...
# ============================================================================
# Lazy singleton service (internal)
# ============================================================================

class _Service:
    """Lazy-initialized service containing heavy components (internal)"""
    _inst: "_Service" = None
...
def translate_paragraph(text: str) -> str:
    """
    Translate a single English paragraph to Hindi.
    
    Args:
        text: English paragraph
    
    Returns:
        Hindi text with English hints
    """
...
def translate_paragraphs(texts: List[str]) -> List[str]:
    """
    Translate multiple English paragraphs to Hindi sequentially.
    Uses internal batching for sentences within each paragraph.
    
    Args:
        texts: List of English paragraphs
    
    Returns:
        List of Hindi translations with English hints
    """
    if not texts:
        return []
...
if __name__ == "__main__":
    ...
    example = (
        "A 73-year-old man presents to the emergency department complaining of abdominal pain with nausea "
        "and vomiting, stating that he 'cannot keep anything down'. He states that the pain has been "
        "gradually getting worse over the past 2 months, saying that, at first, it was present only an hour "
        "after he ate but now is constant."
    )
