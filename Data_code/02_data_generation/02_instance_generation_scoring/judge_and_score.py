...
def judge_entry(entry: dict) -> dict[str, float]:
    ...
    if not ok:
        ...
        return {'grounded_in_context': 0.01, 'medical_correctness': 0.01, 'reasoning_clarity': 0.01, 'language_quality': 0.01}
    try:
        ...
        return {'grounded_in_context': 0, 'medical_correctness': 0, 'reasoning_clarity': 0, 'language_quality': 0}
    ...
    return {'grounded_in_context': _clamp(obj.get('grounded_in_context', 0.0)), 'medical_correctness': _clamp(obj.get('medical_correctness', 0.0)), 'reasoning_clarity': _clamp(obj.get('reasoning_clarity', 0.0)), 'language_quality': _clamp(obj.get('language_quality', 0.0))}
        ...
        if not text_id:
            for entry in text_entries:
                ...
                if isinstance(llmjudge, dict) and llmjudge:
                    scores = [llmjudge.get('grounded_in_context', 0.0), llmjudge.get('medical_correctness', 0.0), llmjudge.get('reasoning_clarity', 0.0), llmjudge.get('language_quality', 0.0)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default=None, help='Path to output JSON file (default: same as input, in-place update).')
